...
<body>
 ...
 <center>
  ...
  <div id=topnavigation style="width:690px;">
     <input type=button class=iButton id=compose value="Compose"><% if (view.equals("inbox")) { %> <input type=button class=iButton id=delete value="Delete">  <% } %>
  </div>

  <form name="mailForm" id="mailForm">
  
  <p><table id=mailtable cellpadding=0 cellspacing=0>
     <thead>
         <tr class=header>
     ...
     <%
      ...
      %>
         <tr class="<%=unread %> messageRow" id="<%=message.id %>">
             <td class="mail"><input <%=disabled %> type=checkbox name="delId" value="<%=message.id%>" class=selectable></td>
             <td class="mail mail-to"><%=message.to%></td>
             <td class="mail mail-from"><%=message.from%></td>
             <td class="mail mail-subj"><%=message.subject%></td>
             <td class="mail mail-date"><%=TimeDisplayUtil.formatTime(message.sentTime, "M/d/y HH:mm", TimeZone.getDefault().toString())%></td>
         </tr>
      <%
            }  // end if statement
        }      // end for loop
...
<div id=composeMessage style="display:none;cursor:default;">
      ...
      <tr><td class=right>Subject</td>
          <td class=left_offset><input class="textfield" type=text name=subject id=subject> <span class=error_message id=subject_error></span></td>
      </tr>
      <tr><td class=right>Message</td>
          <td class=left_offset><textarea class="textfield" name=message id=message style="height:120px;width:400px;"></textarea> <span class=error_message id=message_error></span></td>
      </tr>
      </table>
      <p><input class=iButton type=button id=submitMessage value="Send"> <input class=iButton type=button id=cancelCompose value="Cancel">
      </form>
  </div>
  
 <div id=readMessage style="display:none;cursor:default;">
      <p><h2>Read Message</h2>
      <form id=readMessageForm name=readMessageForm>
      <table width=100% class="content_table" >
      <tr><td class=right>From</td>
      ...
      <tr><td class=right>Message</td>
          <td class=left_offset><textarea name=message id=message class="textfield" style="height:120px;width:400px;"></textarea></td>
      </tr>
      </table>
      <p>
...
<div id=deleteMessage style="display:none;cursor:default;">
      <p><h2>Delete Message</h2>
      <p><span id=deleteConfirm>Are you sure you want to delete this message?</span>
      <p><input class=iButton type=button id=submitDelete value="Yes"> <input class=iButton type=button id=cancelDelete value="No">
</div>
